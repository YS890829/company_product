    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            try:
                ...
                print(f"💡 再認証が必要です: python3 tools/reauth_drive.py")
                raise
        else:
            raise Exception(f"❌ 認証エラー: {TOKEN_PATH} が無効です。\n"
                           f"💡 再認証してください: python3 tools/reauth_drive.py")
