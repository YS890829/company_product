...
def get_events_for_file_date(file_date: str, calendar_id: str = 'primary') -> list:
    ...
    try:
        ...
        print(f"   エラー詳細: {error.resp.status} - {error.error_details if hasattr(error, 'error_details') else 'N/A'}")
        print("   対処方法:")
        print("   1. scripts/diagnose_calendar.py で診断を実行")
        print("   2. token.json を削除して再認証")
        return []
    except Exception as e:
        print(f"❌ 予定取得エラー: {e}")
        import traceback
