...
def main():
    ...
    try:
        ...
        if os.getenv('AUTO_RENAME_FILES', 'false').lower() == 'true':
            try:
                from src.utils.file_management.generate_smart_filename import (
        ...
        if os.getenv('ENABLE_CALENDAR_INTEGRATION', 'false').lower() == 'true':
            try:
                from src.core.calendar_integration import get_file_date, get_events_for_file_date, match_event_with_transcript
        ...
        if os.getenv('ENABLE_INTEGRATED_PIPELINE', 'true').lower() == 'true':
            try:
                from src.services.pipeline.integrated_pipeline import run_phase_11_3_pipeline
                import signal
                ...
                def timeout(seconds):
                    ...
                    def timeout_handler(signum, frame):
                        raise TimeoutError(f"処理がタイムアウトしました ({seconds}秒)")
                ...
                print(f"⏱️  タイムアウト: {pipeline_timeout}秒")

                try:
                    with timeout(pipeline_timeout):
                    ...
                    print(f"⚠️  統合パイプラインがタイムアウトしました: {te}")
                    print(f"   文字起こしは完了しています")
                    print(f"   手動でパイプラインを実行してください:")
                    print(f"   python3 -m src.pipeline.integrated_pipeline {json_path}")
        ...
        if os.getenv('ENABLE_VECTOR_DB', 'true').lower() == 'true' and enhanced_json_path:
            try:
                from src.services.vector_db.build_unified_vector_index import main as build_vector_db
