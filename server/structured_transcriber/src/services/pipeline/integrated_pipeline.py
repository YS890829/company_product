...
Step 9: 参加者DB更新
Step 10: 会議情報登録
Step 11: Enhanced JSON保存 ★Phase 11-4追加

更新履歴:
- 2025-10-16: Phase 11-3初版
- 2025-10-17: トピック/エンティティ抽出最適化
- 2025-10-18: Step 11 Enhanced JSON保存機能追加 (Phase 11-4)
"""
...
from src.services.participants.participants_db import ParticipantsDB
from src.services.participants.extract_participants import extract_participants_from_description
from src.services.participants.enhanced_speaker_inference import infer_speakers_with_participants, apply_speaker_inference_to_structured_json
from src.core.calendar_integration import get_events_for_file_date, match_event_with_transcript
from src.core.summary_generator import generate_summary_with_calendar
from src.services.topics.add_topics_entities import extract_topics_and_entities
...
def run_phase_11_3_pipeline(structured_file_path: str) -> Dict:
    ...
    matched_event = None
    calendar_match_failed_reason = None
    try:
        ...
        if events:
            ...
            if matched_event:
                ...
                calendar_match_failed_reason = "マッチング失敗（該当イベントなし）"
                print(f"  ⚠ {calendar_match_failed_reason}")
        else:
            calendar_match_failed_reason = f"{file_date}にカレンダーイベントなし"
            print(f"  ⚠ {calendar_match_failed_reason}")
            print(f"  ℹ️  診断コマンド: python scripts/diagnose_calendar.py {file_date}")
    except Exception as e:
        calendar_match_failed_reason = f"カレンダーAPIエラー: {e}"
        print(f"  ⚠ {calendar_match_failed_reason}")
        import traceback
        traceback.print_exc()
    ...
    # 優先順位1: カレンダーから抽出された参加者
    if calendar_participants:
        print("  ℹ️  参加者ソース: カレンダーイベント")
        for p in calendar_participants:
            canonical_name = p.get("canonical_name", "")
            ...
            print(f"  ✓ {canonical_name}: DB更新完了（カレンダー）")

    # 優先順位2: 会話内容から抽出された人物（エンティティ）
    elif resolved_people:
        print("  ℹ️  参加者ソース: 会話内容（エンティティ抽出）")
        print(f"  ℹ️  抽出された人物: {len(resolved_people)}名")
        ...
        for person in resolved_people:
            ...
            # 会話内容から抽出した参加者を登録
            notes = f"会話内容から自動抽出（カレンダー情報なし） - {datetime.now().strftime('%Y-%m-%d')}"
            ...
            participant_canonical_names.append(person)
            print(f"  ✓ {person}: DB更新完了（会話内容）")

    else:
        print("  ⏭ スキップ（参加者情報なし - カレンダー・会話内容ともに該当なし）")
    ...
    enhanced_json = {
        "metadata": {
            "original_file": structured_file_path,
            "meeting_id": meeting_id,
            "meeting_date": file_date,
            "processed_at": datetime.now().isoformat()
        },
        "meeting": {
            "title": matched_event.get('summary', '無題') if matched_event else 'カレンダーイベントなし',
            "calendar_event_id": matched_event.get('id') if matched_event else None,
            "start_time": matched_event.get('start', {}).get('dateTime') if matched_event else None,
            "end_time": matched_event.get('end', {}).get('dateTime') if matched_event else None
        },
        "participants": {
            "calendar_participants": calendar_participants,
            "canonical_names": participant_canonical_names
        },
        "content": {
            "topics": topics,
            "entities": {
        ...
        "segments": segments  # 話者推論適用済みセグメント
    }

    # 保存パスの生成
    enhanced_file_path = structured_file_path.replace('_structured.json', '_structured_enhanced.json')

    # Enhanced JSON保存
    with open(enhanced_file_path, 'w', encoding='utf-8') as f:
        json.dump(enhanced_json, f, ensure_ascii=False, indent=2)

    print(f"  ✓ Enhanced JSON保存完了: {os.path.basename(enhanced_file_path)}")
    print(f"  ✓ ファイルサイズ: {os.path.getsize(enhanced_file_path) / 1024:.1f} KB")

    # ========================
    # 完了
    # ========================
    print(f"\n{'='*60}")
    print(f"[Phase 11-3 + 11-4] パイプライン完了")
    print(f"{'='*60}\n")
