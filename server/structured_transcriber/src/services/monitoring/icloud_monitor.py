...
def process_new_audio_file(file_path: Path):
    ...
    try:
        ...
        # 3-1. ファイル作成時刻を取得 (st_birthtime)
        created_time = None
        try:
            stat_info = file_path.stat()
            # macOS: st_birthtime は作成時刻
            created_timestamp = stat_info.st_birthtime
            created_time = datetime.fromtimestamp(created_timestamp, tz=timezone.utc).isoformat()
            print(f"  📅 File created at: {created_time}", flush=True)
        except Exception as e:
            print(f"  ⚠️ Could not get file creation time: {e}", flush=True)

        # 4. 重複チェック（ユーザー表示名 + original_name + created_time ベース）
        # Updated: Now checks composite key (display_name + original_name + created_time)
        if registry.is_processed(user_display_name, file_path.name, created_time):
            existing = registry.get_by_display_name(user_display_name, file_path.name, created_time)
...
def write_health_status(status: str = "running"):
    """
    Write health status to a file for monitoring

    Args:
        status: Status string (running, error, stopped)
    """
    import json
    from datetime import datetime, timezone

    health_file = Path('.icloud_monitor_health.json')
    health_data = {
        'status': status,
        'timestamp': datetime.now(timezone.utc).isoformat(),
        'monitoring_path': str(ICLOUD_PATH),
        'pid': os.getpid()
    }
    ...
    try:
        ...
        print(f"Warning: Could not write health status: {e}")
