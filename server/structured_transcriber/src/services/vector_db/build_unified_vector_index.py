...
class UnifiedVectorIndexBuilder:
    ...
    def prepare_unified_documents(self, json_files: List[str]) -> tuple[List[str], List[Dict[str, Any]], List[str]]:
        ...
        for json_file in json_files:
            ...
            # ソースファイル名（Enhanced JSONファイル名から取得）
            # 例: "20251015_育児_子供の様子_structured_enhanced.json" → "20251015_育児_子供の様子.m4a"
            json_basename = Path(json_file).stem  # "_structured_enhanced"を除く
            source_file = json_basename.replace('_structured_enhanced', '') + '.m4a'
            ...
            for segment in segments:
                ...
                # ファイル名全体を使用（切り詰めない）
                file_prefix = Path(json_file).stem.replace('_structured_enhanced', '')
                unique_id = f"{file_prefix}_seg_{segment_id}"

                # メタデータ作成
                metadata = {
                    'segment_id': str(segment_id),
                    'file_name': source_file,  # 検索用のファイル名フィールド
                    'source_file': source_file,  # 後方互換性のため維持
    ...
    def build_unified_index(self, texts: List[str], metadatas: List[Dict[str, Any]],
                           ids: List[str], collection_name: str = "transcripts_unified",
        ...
        Args:
            ...
            append_mode: Trueなら既存コレクションに追加、Falseなら削除して新規作成
        """
        ...
        if append_mode:
            try:
                ...
                print(f"   Existing collection found: {existing_count} embeddings")
            except Exception:
                print(f"   No existing collection, creating new one")
        ...
        print(f"   Documents added this run: {len(texts)}")
        print(f"   Collection: {collection_name}")
...
def main(json_files=None, append_mode=True):
    """
    メイン処理

    Args:
        json_files: enhanced JSONファイルのリスト（Noneの場合はコマンドライン引数から取得）
        append_mode: Trueなら既存コレクションに追加、Falseなら削除して新規作成
    """
    ...
    # 統合ベクトルインデックス構築
    builder.build_unified_index(texts, metadatas, ids, collection_name="transcripts_unified", append_mode=append_mode)
